...
export function DeleteFacultyDialog({ isOpen, onClose, onSuccess, faculty }: DeleteFacultyDialogProps) {
  const [confirmText, setConfirmText] = useState('')
  const [error, setError] = useState('')

  const [deleteFaculty, { loading }] = useMutation(DELETE_FACULTY, {
    onCompleted: () => {
      onSuccess()
      setConfirmText('')
      setError('')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (confirmText !== faculty.id) {
      setError('Mã khoa không khớp. Vui lòng nhập chính xác để xác nhận xóa.')
      return
    }
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
            ...
            <p className="text-sm text-red-700 dark:text-red-300">
              Xóa khoa sẽ xóa tất cả chuyên ngành thuộc khoa này và ảnh hưởng đến sinh viên, giáo viên liên quan.
            </p>
          </div>
          ...
          <div className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg space-y-2">
            <div>
              <span className="text-sm text-gray-500 dark:text-gray-400">Mã khoa:</span>
              <p className="font-mono font-medium text-gray-900 dark:text-gray-100">
                {faculty.id}
              </p>
            </div>
            <div>
              <span className="text-sm text-gray-500 dark:text-gray-400">Tên khoa:</span>
          ...
          <div>
            <label htmlFor="confirmText" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Nhập mã khoa <code className="px-1 py-0.5 bg-gray-100 dark:bg-gray-700 rounded text-red-600 dark:text-red-400">{faculty.id}</code> để xác nhận
            </label>
            <input
              id="confirmText"
              type="text"
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button
              ...
              disabled={loading || confirmText !== faculty.id}
            >
