...
export function DeleteStudentDialog({ isOpen, onClose, onSuccess, student }: DeleteStudentDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (confirmText !== student.id) {
      setError('Mã sinh viên không khớp. Vui lòng nhập chính xác để xác nhận xóa.')
      return
    }
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
            ...
            <p className="text-sm text-red-700 dark:text-red-300">
              Xóa sinh viên sẽ xóa tất cả dữ liệu liên quan bao gồm đề tài, điểm số và các hoạt động khác.
            </p>
          </div>
          ...
          <div>
            <label htmlFor="confirmText" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Nhập mã sinh viên <code className="px-1 py-0.5 bg-gray-100 dark:bg-gray-700 rounded text-red-600 dark:text-red-400">{student.id}</code> để xác nhận
            </label>
            <input
              id="confirmText"
              type="text"
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button
              ...
              disabled={loading || confirmText !== student.id}
            >
              {loading ? 'Đang xóa...' : 'Xóa sinh viên'}
            </button>
          </div>
        </form>
      </div>
